...
$sourceDir = Join-Path $root "src\DonChanZombieRadar"
$outputDir = Join-Path $root "mod\DonChanZombieRadar"
$outputDll = Join-Path $outputDir "DonChanZombieRadar.dll"
$outputPdb = Join-Path $outputDir "DonChanZombieRadar.pdb"
...
$unityImgui = Join-Path $managedDir "UnityEngine.IMGUIModule.dll"
$unityInput = Join-Path $managedDir "UnityEngine.InputLegacyModule.dll"
$unityText = Join-Path $managedDir "UnityEngine.TextRenderingModule.dll"
...
foreach ($required in @($assembly, $logLibrary, $unityCore, $unityImgui, $unityInput)) {
...
foreach ($r in @($assembly, $harmony, $logLibrary, $unityCore, $unityImgui, $unityInput)) { if (!$references.Contains($r)) { $references.Add($r) } }
if (Test-Path $unityEngine) { if (!$references.Contains($unityEngine)) { $references.Add($unityEngine) } }
if (Test-Path $unityText) { if (!$references.Contains($unityText)) { $references.Add($unityText) } }
...
$rsp = Join-Path $env:TEMP ("DonChanZombieRadar-" + [Guid]::NewGuid().ToString("N") + ".rsp")

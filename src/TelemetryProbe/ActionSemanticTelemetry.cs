...
namespace DonChan.TelemetryProbe
{
    ...
    internal static class SemanticRangedFiredPatch
    {
        private static void Postfix(ItemActionData _actionData)
        {
            ...
            {
                ActionSemanticTelemetry.Emit("action.deployable.use", "Deployable", SemanticActionClassifier.DeployableSubtype(item), item, itemValue, null);
                return;
            }
